#!/usr/bin/env python3
"""Sanity-check the generated Nantong guide.
...
HTML = os.path.join(ROOT, "南通3天2夜攻略.html")
...
def main():
    ...
    for f in sorted(os.listdir(os.path.join(ROOT, "images"))):
        if not f.endswith((".jpg", ".png")):
            continue
        if not re.match(r"(nantong|hero-nantong|langshan|haohe|tangzha|zhonglou|tianning|wenfeng|sutong|hutong|binjiang|zhangjian|shi-street|rugao|yuantuo|lvsi|qidong|haimen|rudong|jianghai|wildlife|lvboyuan|yangtze|nantong-skyline)", f):
            continue
        rel = "images/" + f
        if rel not in refs:
            unused.append(rel)
    if unused:
        warns.append(f"Nantong images downloaded but not used: {unused}")
